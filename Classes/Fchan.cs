...
namespace YChanEx {
    class Fchan : ImageBoard {
        ...
        override public void download() {
            ...
            try {
                Uri threadURI = new Uri(this.getURL());
                ...
                var srcstream = returnedSource.GetResponseStream();
                var reader = new StreamReader(srcstream);
                website = reader.ReadToEnd();
                srcstream.Flush();
                srcstream.Close();
                reader.Close();
                returnedSource.Close();
                ...
                string json = wc.DownloadString(JURL);
                byte[] bytes = Encoding.ASCII.GetBytes(json);
                using (var stream = new MemoryStream(bytes)) {
                    var quotas = new XmlDictionaryReaderQuotas();
                    var jsonReader = JsonReaderWriterFactory.CreateJsonReader(stream, quotas);
                ...
                for (int y = 0; y < URLs.Length - 1; y++)
                    if (YCSettings.Default.originalName) {
                        ...
                        FileController.downloadFile(URLs[y], this.SaveTo, true, newfilename);
                        website = website.Replace(URLs[y].Split('/')[4], newfilename + xmlExt[y].InnerText);
                    }
                    else {
                        FileController.downloadFile(URLs[y], this.SaveTo, false, string.Empty);
                    }
                ...
                if (YCSettings.Default.htmlDownload == true && website != "")
                    FileController.downloadHTML(false, website.Replace("class=\"fileThumb\" href=\"thumb/", "class=\"fileThumb\" href=\""), this.SaveTo);
            }
            catch (WebException webEx) {
                Debug.Print(webEx.ToString());
                if (((int)webEx.Status) == 7)
