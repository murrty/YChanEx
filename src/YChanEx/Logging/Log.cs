...
//#if !DEBUG
            ...
            AppDomain.CurrentDomain.UnhandledException += (sender, exception) => {
                using frmException UnrecoverableException = new(new((Exception)exception.ExceptionObject) {
                    Unrecoverable = true,
                    AllowRetry = false
                });
            ...
            Application.ThreadException += (sender, exception) => {
                using frmException UnrecoverableException = new(new(exception.Exception) {
                    Unrecoverable = true,
                    AllowRetry = false
                });
                UnrecoverableException.ShowDialog();
...
//#endif
    ...
    /// <param name="ExtraInfo">Any extra information regarding the exception.</param>
    /// <returns>The <see cref="DialogResult"/> of the exception form.</returns>
    public static DialogResult ReportRetriableLanguageException(Exception ReceivedException, object ExtraInfo = null) {
        Write($"A {ReceivedException.GetType().Name} occurred.");
