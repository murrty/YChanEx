...
[ToolboxItem(false)]
[Obsolete("Convert to HttpClient.")]
internal class ExtendedWebClient : WebClient {
    ...
    /// </summary>
    public DateTime ResourceLastModified { get; private set; }
    ...
    /// </summary>
    public WebRequest BuiltRequest { get; private set; }

    /// <summary>
    /// Gets the received <see cref="WebResponse"/> from the online resource after receiving header data.
    /// </summary>
    public WebResponse ReceivedResponse { get; private set; }

    /// <summary>
    /// Gets or sets the permanent headers for the request.
    /// </summary>
    public Dictionary<string, string> PermanentHeaders { get; set; } = [];
    #endregion
    ...
    [System.Diagnostics.DebuggerHidden]
    [System.Diagnostics.CodeAnalysis.SuppressMessage("Roslynator", "RCS1047:Non-asynchronous method name should not end with 'Async'")]
    public new void DownloadFileAsync(Uri Address, string FileName, object Token) {
        string Dir = System.IO.Path.GetDirectoryName(FileName);
        if (!System.IO.Directory.Exists(Dir)) {
            System.IO.Directory.CreateDirectory(Dir);
        }
        base.DownloadFileAsync(Address, FileName, Token);
    }
    ...
    private WebRequest ManageRequest(WebRequest Request) {
        ...
        // Add the permanent headers, these take precident.
        if (PermanentHeaders.Count > 0) {
            foreach (var header in PermanentHeaders) {
