...
public sealed class Config_Advanced {
    ...
    public void Save() {
        if (UserAgent != fUserAgent)
            fUserAgent = IniProvider.Write(UserAgent, ConfigName);
        if (DisableScanWhenOpeningSettings != fDisabledScanWhenOpeningSettings)
            fDisabledScanWhenOpeningSettings = IniProvider.Write(DisableScanWhenOpeningSettings, ConfigName);
        if (SilenceErrors != fSilenceErrors)
            fSilenceErrors = IniProvider.Write(SilenceErrors, ConfigName);
    }
