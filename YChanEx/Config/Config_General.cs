...
public sealed class Config_General {
    ...
    public void Save() {
        if (ShowTrayIcon != fShowTrayIcon)
            fShowTrayIcon = IniProvider.Write(ShowTrayIcon, ConfigName);
        if (MinimizeToTray != fMinimizeToTray)
            fMinimizeToTray = IniProvider.Write(MinimizeToTray, ConfigName);
        if (ShowExitWarning != fShowExitWarning)
            fShowExitWarning = IniProvider.Write(ShowExitWarning, ConfigName);
        if (EnableUpdates != fEnableUpdates)
            fEnableUpdates = IniProvider.Write(EnableUpdates, ConfigName);
        if (UseFullBoardNameForTitle != fUseFullBoardNameForTitle)
            fUseFullBoardNameForTitle = IniProvider.Write(UseFullBoardNameForTitle, ConfigName);
        if (SaveQueueOnExit != fSaveQueueOnExit)
            fSaveQueueOnExit = IniProvider.Write(SaveQueueOnExit, ConfigName);
        if (MinimizeInsteadOfExiting != fMinimizeInsteadOfExiting)
            fMinimizeInsteadOfExiting = IniProvider.Write(MinimizeInsteadOfExiting, ConfigName);
        if (AutoSaveThreads != fAutoSaveThreads)
            fAutoSaveThreads = IniProvider.Write(AutoSaveThreads, ConfigName);
        if (SaveThreadHistory != fSaveThreadHistory)
            fSaveThreadHistory = IniProvider.Write(SaveThreadHistory, ConfigName);
    }
