...
public sealed class Config_Regex {
    ...
    public void Save() {
        if (fchanFiles != ffchanFiles)
            ffchanFiles = IniProvider.Write(fchanFiles, ConfigName);
        if (fchanIDs != ffchanIDs)
            ffchanIDs = IniProvider.Write(fchanIDs, ConfigName);
        if (u18chanFiles != fu18chanFiles)
            fu18chanFiles = IniProvider.Write(u18chanFiles, ConfigName);
        if (u18chanID != fu18chanID)
            fu18chanID = IniProvider.Write(u18chanID, ConfigName);
        if (u18chanPosts != fu18chanPosts)
            fu18chanPosts = IniProvider.Write(u18chanPosts, ConfigName);
    }
